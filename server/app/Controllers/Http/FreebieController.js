...
class FreebieController {
  ...
  async store({ request, response, auth }) {
    const numbers = [
      0.01, 0.03, 0.05, 0.1, 0.15, 0.18, 0.2, 0.01, 0.03, 0.05, 0.1, 0.15, 0.18,
      0.2,
    ]
    const index = Math.floor(Math.random() * numbers.length) // generate a random index number
    const amount = numbers[index]
