...
io.of('/mines')
  ...
  .on('connection', (socket) => {
    socket.on('mines:betPlace', async (data) => {
      const user =
        socket.user && socket.user.length ? socket.user : await getUser(socket)
      try {
    ...
    socket.on('mines:cashoutWin', async (data) => {
      const user =
        socket.user && socket.user.length ? socket.user : await getUser(socket)
      try {
        user.balance += data.placeAmount
        await user.save()
      } catch (e) {
        console.log(e)
      }
